-- Pago mixto: repartir efectivo / transferencia en agregados admin.
  ...
  select json_build_object(
    ...
    'cashCents',
      coalesce(sum(
        case when o.status = 'paid' then
          case
            when trim(coalesce(o.wompi_reference, '')) = 'POS:cash'
              then greatest(0, coalesce(o.total_cents, 0))
            when trim(coalesce(o.wompi_reference, '')) = 'POS:mixed'
              then greatest(0, coalesce(o.pos_mixed_cash_cents, 0))
            else 0
          end
        else 0 end
      ), 0),
    'transferCents',
      coalesce(sum(
        case when o.status = 'paid' then
          case
            when trim(coalesce(o.wompi_reference, '')) = 'POS:transfer'
              then greatest(0, coalesce(o.total_cents, 0))
            when trim(coalesce(o.wompi_reference, '')) = 'POS:mixed'
              then greatest(0, coalesce(o.pos_mixed_transfer_cents, 0))
            else 0
          end
        else 0 end
      ), 0),
    'mixedCents',
      coalesce(sum(
        case when o.status = 'paid'
          ...
          and o.pos_mixed_cash_cents is null
          and o.pos_mixed_transfer_cents is null
          then greatest(0, coalesce(o.total_cents, 0))
          else 0 end
      ), 0),
    'otherCents',
...
-- Actualiza reportes: mixto ya no suma todo en transferencia.
