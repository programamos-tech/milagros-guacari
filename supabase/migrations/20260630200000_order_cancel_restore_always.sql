-- Anulación: devolver stock aunque el pedido ya no esté en paid (p. ej. pasó a pendiente en UI).
-- Solo idempotencia vía stock_restored_at; no exigir status = paid.
...
comment on function public.restore_order_items_stock(uuid) is
  'Devuelve inventario al anular (idempotente). No requiere status paid: POS puede haber descontado aunque el estado cambió en UI.';
