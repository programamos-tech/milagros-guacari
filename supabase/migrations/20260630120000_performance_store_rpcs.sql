-- Tienda: una query para browse por categoría, conteos y facets agregados.
-- Admin: stock reservado por producto.
...
-- Top-N productos publicados por category_id (incluye category_id NULL).
...
grant execute on function public.store_published_product_counts_by_category() to anon, authenticated;

/**
 * Facetas de listado: marcas, colores, tallas distintas y rango de precio.
 * p_category_ids NULL = catálogo completo.
 */
...
grant execute on function public.store_listing_facets_agg(uuid[]) to anon, authenticated;

create or replace function public.admin_product_reserved_quantity(p_product_id uuid)
returns bigint
language sql
stable
security invoker
...
grant execute on function public.admin_product_reserved_quantity(uuid) to authenticated;
