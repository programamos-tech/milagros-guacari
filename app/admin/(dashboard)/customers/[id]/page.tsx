...
export default async function AdminCustomerDetailPage({ params, searchParams }: Props) {
  ...
  const trendPct = ticketTrendMonthOverMonthPercent(ticketTrendPoints);
  ...
  const metricCardClass =
    "flex flex-col gap-1.5 rounded-xl border border-rose-200/50 bg-gradient-to-b from-white to-rose-50/30 p-4 shadow-sm dark:border-zinc-700/80 dark:from-zinc-900 dark:to-zinc-900/80 dark:shadow-none";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className={`${adminPanelLgClass} overflow-hidden`}>
        <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 flex-1 flex-col gap-5 sm:flex-row sm:items-center sm:gap-6">
            <CustomerAvatar
              seed={avatarSeed}
              size={112}
              className="shadow-md ring-2 ring-rose-200/80 dark:ring-zinc-600"
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="inline-flex rounded-md bg-rose-600 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-white dark:bg-rose-500">
                  Cliente
                </span>
                {ventas > 0 ? (
                  <span
          ...
          <div className="flex w-full flex-col gap-4 sm:flex-row sm:items-start sm:justify-end lg:w-auto lg:flex-col lg:items-end">
            {ventas > 0 ? (
              <div
                className="flex items-center gap-3 rounded-xl border border-rose-200/70 bg-rose-50/80 px-4 py-3 dark:border-rose-900/40 dark:bg-rose-950/25"
                title="Referencia visual: miles de pesos gastados (total ÷ $10.000)."
              >
                <IconMedal className="size-8 shrink-0 text-rose-600 dark:text-rose-400" />
                <div>
                  <p className="text-2xl font-semibold tabular-nums text-rose-950 dark:text-rose-100">
                    {puntosRef.toLocaleString("es-CO")}{" "}
                    <span className="text-sm font-medium text-rose-800/90 dark:text-rose-200/90">puntos</span>
                  </p>
                  <p className="text-[11px] text-rose-800/70 dark:text-rose-300/80">Según volumen de compra</p>
                </div>
              </div>
            ) : null}
        ...
        <div className="border-t border-rose-100/80 px-4 py-5 dark:border-zinc-800 sm:px-8">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <div className={metricCardClass}>
              <IconBag className="size-6 text-rose-600 dark:text-rose-400" />
              <p className={labelClass}>Compras</p>
              <p className="text-xl font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">{ventas}</p>
            </div>
            <div className={metricCardClass}>
              <IconTruck className="size-6 text-rose-600 dark:text-rose-400" />
            ...
            <div className={metricCardClass}>
              <IconStore className="size-6 text-rose-600 dark:text-rose-400" />
            ...
            <div className={metricCardClass}>
              <IconCoin className="size-6 text-rose-600 dark:text-rose-400" />
            ...
            <div className={metricCardClass}>
              <IconClock className="size-6 text-rose-600 dark:text-rose-400" />
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
            <div>
              ...
              <p className="mt-1 max-w-xl text-sm text-zinc-500 dark:text-zinc-400">
                Ticket promedio por mes (ventas pagadas). Útil para ver si el gasto por compra sube o baja.
              </p>
            </div>
            {trendPct != null ? (
              <div
                className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold tabular-nums ring-1 ${
                  trendPct >= 0
                    ? "bg-emerald-50 text-emerald-900 ring-emerald-200/90 dark:bg-emerald-950/40 dark:text-emerald-100 dark:ring-emerald-800/55"
                    : "bg-rose-50 text-rose-900 ring-rose-200/90 dark:bg-rose-950/35 dark:text-rose-100 dark:ring-rose-900/50"
                }`}
              >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {customer.email?.trim() ? (
                <a
                  ...
                  className="flex items-start gap-3 rounded-xl border border-zinc-100 bg-zinc-50/50 p-3 transition hover:border-rose-200/80 hover:bg-rose-50/40 dark:border-zinc-800 dark:bg-zinc-950/40 dark:hover:border-zinc-600 dark:hover:bg-zinc-800/50"
                >
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-rose-100 text-rose-700 dark:bg-rose-950/50 dark:text-rose-300">
              ...
              {customer.document_id?.trim() ? (
                <div className="flex items-start gap-3 rounded-xl border border-zinc-100 bg-zinc-50/50 p-3 dark:border-zinc-800 dark:bg-zinc-950/40">
              ...
              {fullAddressText ? (
                <div className="sm:col-span-2 flex items-start gap-3 rounded-xl border border-zinc-100 bg-zinc-50/50 p-3 dark:border-zinc-800 dark:bg-zinc-950/40">
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-zinc-200/80 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    <svg viewBox="0 0 24 24" className="size-5" fill="none" stroke="currentColor" strokeWidth={1.75} aria-hidden>
            ...
            {topProducts.length === 0 ? (
              ...
              <ol className="mt-5 space-y-3">
                {topProducts.slice(0, 8).map((row, i) => (
                  <li
                    key={`${row.name}-${i}`}
                    className="flex items-start gap-3 rounded-xl border border-zinc-100/90 bg-white/60 px-3 py-3 dark:border-zinc-800 dark:bg-zinc-950/30"
                  >
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-rose-600 text-xs font-bold text-white dark:bg-rose-500">
                      {i + 1}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                        ...
                        {" · "}
                        <span className="tabular-nums text-rose-700 dark:text-rose-300">{formatCop(row.totalCents)}</span>
                      </p>
                    </div>
                  </li>
                ))}
                ...
                return (
                  <li key={o.id}>
                    <Link
                      ...
                      className="flex flex-col gap-2 rounded-xl border border-zinc-100 bg-zinc-50/40 p-4 transition hover:border-rose-200/70 hover:bg-rose-50/30 dark:border-zinc-800 dark:bg-zinc-950/40 dark:hover:border-zinc-600 dark:hover:bg-zinc-800/40"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex min-w-0 items-start gap-2">
                          <span className="mt-0.5 text-rose-600 dark:text-rose-400" aria-hidden>
      ...
      <section id="historial" className={`${adminPanelLgClass} p-6 sm:p-8`}>
        <h2 className="text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-400 dark:text-zinc-500">
          Historial de pedidos
        </h2>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          Todos los estados (incluye pendientes y cancelados).{" "}
          <span className="tabular-nums font-medium text-zinc-700 dark:text-zinc-300">{customerOrders.length}</span> en total.
        </p>
